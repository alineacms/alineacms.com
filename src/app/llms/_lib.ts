...
export async function getDocsEntries() {
  ...
  return [docsRoot, ...docsChildren].filter(Boolean) as Array<LlmsDocEntry>
}
...
export async function getLlmsRenderData() {
  ...
  return {docsEntries, entryMap, mediaMap}
}

export function getEntryTitle(entry: LlmsDocEntry) {
  return normalizeText(entry.navigationTitle || entry.title || 'Untitled')
}
...
export function renderDocBody(
  ...
  return renderNodes(entry.body, entryMap, mediaMap)
}

export function toLlmsMdPath(url: string) {
  const segments = url
    .split('/')
    .filter(Boolean)
    .map(segment => encodeURIComponent(segment))
  return `/llms/${segments.join('/')}.md`
}
